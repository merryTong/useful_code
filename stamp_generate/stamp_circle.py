...
class Stamp:
    def __init__(self,  edge = 5,               # 图片边缘空白的距离
                        ...
                        border = 13,            # 字到圆圈内侧的距离
                        r = 90,                 # 五星外接圆半径
                        fill = (255, 0, 0, 255),# 印章颜色， 默认纯红色， 透明度0-255，建议90-180
    ...
    def draw_stamp(self):
        ...
        for word in self.words_mid:
            self.draw_rotated_text(img, 0, (self.R+self.edge+ self.H*tan(angle_word_curr*pi/180),self.R+self.edge), self.H,
                                   word, self.fill, self.font_size_mid, self.font_xratio_mid, self.stroke_width_mid, font_flip = True)
            angle_word_curr = angle_word_curr + angle_word
...
my_fake = Faker("zh-CN")
def generate_agent_name(my_fake):
    up,mid,down = "","",""
    company_name = my_fake.company()
    city = my_fake.city()
    up = city+company_name if len(city+company_name) < 15 else company_name

    country = my_fake.country()
    mid = country if len(country) <= 7 else country[:6]

    leng = randint(8,13)
    down = my_fake.credit_card_number(card_type=None)
